...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
      ...
      try {
        ...
        if (validation.success) {
          const results = await Promise.all([
            ...
            // Phase 4 Fetches
            fetch(getApiUrl("/api/velocity"))
              .then((r) => r.json())
              .catch(() => ({})),
            fetch(getApiUrl("/api/win-loss"))
              .then((r) => r.json())
              .catch(() => ({})),
          ...
          if (isMounted) {
            ...
            setMetrics({
              workflows: wf,
              sequences: seq,
              pipelines: [], // Placeholder for future use
              properties: prop,
              segments: seg,
              campaigns: camp,
              forms,
  ...
  const StatCard = ({
    ...
    return (
      ...
      >
        <div
          className={`absolute -right-6 -top-6 w-32 h-32 ${gradient} opacity-10 rounded-full blur-3xl group-hover:opacity-20 transition-opacity`}
        ></div>

        <div className="relative z-10">
          <div className="flex justify-between items-center mb-6">
            <div
              className={`p-4 rounded-2xl bg-slate-100 border border-slate-200 text-slate-900 shadow-xl`}
            >
              <Icon size={28} />
            </div>
            <div className="text-[10px] font-bold text-slate-900/40 uppercase tracking-[0.2em]">
              {active ? "Live" : "Cached"}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <h2 className="text-sm font-bold text-slate-600 mt-2 uppercase tracking-wider">
              {title}
            </h2>
          </div>
          ...
          <div className="mt-6 pt-6 border-t border-slate-200 flex items-center justify-between">
            ...
            {onClick && (
              <ArrowUpRight
                size={14}
                className="text-slate-600 group-hover:text-slate-900 group-hover:scale-125 transition-all"
              />
            )}
          </div>
        </div>
  ...
  const winRate =
    metrics.winLoss?.cohorts?.won &&
    metrics.winLoss.cohorts.won.count +
      (metrics.winLoss.cohorts.lost?.count || 0) >
      0
      ? (
          (metrics.winLoss.cohorts.won.count /
            (metrics.winLoss.cohorts.won.count +
      ...
      : "0.0";
  const topAssetName =
    metrics.topAssets.length > 0 ? metrics.topAssets[0].form_name : "N/A";
  const topAssetRev =
    metrics.topAssets.length > 0 ? metrics.topAssets[0].total_revenue : 0;
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-600 max-w-lg font-medium leading-relaxed">
            Unified view of your revenue architecture. Converting raw activity
            into strategic velocity.
          </p>
        </div>
      ...
      {metrics.analytics.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {metrics.analytics.map((m: any, i: number) => (
            <div
              key={i}
              className="glass-card p-5 border-slate-200 hover:border-indigo-500/20 transition-all"
            >
        ...
        <div className="lg:col-span-1">
          <StatCard
            ...
            value={
              loading
                ? "..."
                : topAssetName.length > 15
                  ? topAssetName.substring(0, 15) + "..."
                  : topAssetName
            }
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-black text-slate-900 italic tracking-tighter uppercase">
                Revenue <span className="gradient-text">Architecture.</span>
              </h2>
              <p className="text-slate-600 text-xs font-bold uppercase tracking-widest mt-1">
                Pipeline Stage Distribution & Heatmap
              </p>
            </div>
            <div className="flex gap-4">
              <div className="text-right">
                ...
                <span className="text-lg font-black text-slate-900">
                  {metrics.velocity?.components?.openOpportunities || 0}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="glass-card p-8 bg-indigo-500/5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-black text-slate-900 text-sm uppercase tracking-widest italic">
                RevOps Audit
              </h3>
            </div>
            <div className="space-y-4">
              {metrics.revopsAudit?.bottlenecks
                ?.slice(0, 2)
                .map((b: any, i: number) => (
      ...
      {isConnected &&
        !loading &&
        metrics.contactHealth.lifecycleStageBreakdown && (
          <div className="mt-8">
            <JourneyFunnel
              data={metrics.contactHealth.lifecycleStageBreakdown}
            />
          </div>
        )}
