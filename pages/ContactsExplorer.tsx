...
const ContactsExplorer: React.FC = () => {
  ...
  const [noteStatus, setNoteStatus] = useState<
    "idle" | "generating" | "saving" | "saved" | "error"
  >("idle");
  ...
  const isMeaningfulTag = (value: string | null) => {
    ...
    if (/^(user(id)?|contact(id)?|vid|id)[:_ ]?\d+$/i.test(trimmed))
      return false;
    return true;
  };
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      else if (activeSegment === "nurture")
        filterParams.classification = "Nurture";
      else if (activeSegment === "watch") filterParams.classification = "Watch";
      else if (activeSegment === "new") filterParams.classification = "New";
      else if (activeSegment === "unqualified")
        filterParams.classification = "Unqualified";
      else if (activeSegment === "customer") filterParams.activeClient = "true";
      else if (activeSegment === "opportunity")
        filterParams.lifecyclestage = "opportunity";
      else if (activeSegment === "closed-lost")
        filterParams.dealStage = "closedlost";
      ...
      if (selectedFilter === "unassigned") filterParams.hasOwner = "false";
        ...
        filterParams.classification = selectedFilter;
...
Output only the note body, no markdown.`
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (/EXPIRED_AUTHENTICATION/i.test(errText) || resp.status === 401) {
          throw new Error(
            "HubSpot token expired — click “Connect HubSpot” (top-right) and try again."
          );
        }
  ...
  const handleQuickStageChange = async (contactId: string, stage: string) => {
    setUpdatingStageId(contactId);
    try {
      const accessToken = localStorage.getItem("hubspot_access_token");
      const resp = await fetch(getApiUrl("/api/remediate"), {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "remediate-contact",
          hubspotToken: accessToken,
          payload: {
            contactId,
            updates: { lifecyclestage: stage },
            reason: `Manual one-click stage change to ${stage}`,
          },
        }),
      });
      ...
      if (resp.ok) {
        ...
        setContacts((prev) =>
          prev.map((c) =>
            c.id === contactId ? { ...c, lifecyclestage: stage } : c
          )
        );
        if (selectedContact && selectedContact.id === contactId) {
          setSelectedContact((prev) =>
            prev ? { ...prev, lifecyclestage: stage } : null
          );
        }
      } else {
  ...
  const buildLeadSignals = (contact: ContactDetails) => {
    ...
    addSignal(
      "Conversions",
      props.num_conversion_events || props.num_conversion_events
    );
    addSignal(
      "Last Visit",
      props.hs_analytics_last_visit_timestamp
        ? formatDate(props.hs_analytics_last_visit_timestamp)
        : null
    );
    addSignal(
      "Last Modified",
      contact.last_modified ? formatDate(contact.last_modified) : null
    );
  ...
  const handleContactOpen = async (contact: Contact) => {
    ...
    try {
      const resp = await fetch(
        `${getApiUrl("/api/contacts")}?id=${contact.id}`
      );
      if (resp.ok) {
  ...
  return (
    <div className="flex bg-[#f8fafc] text-slate-900 min-h-screen font-['Inter'] w-full overflow-hidden min-w-0">
      ...
      <main className="flex-1 p-10 overflow-y-auto max-h-screen min-w-0 overflow-x-hidden">
            ...
            <h1 className="text-6xl font-black tracking-tighter leading-none mb-4 uppercase">
              {activeSegment === "all"
                ...
                : activeSegment === "hot"
                  ...
                  : activeSegment === "new"
                    ? "New Leads"
                    : activeSegment === "nurture"
                      ? "Nurture"
                      : activeSegment === "watch"
                        ? "Watch List"
                        : activeSegment === "unqualified"
                          ? "Unqualified"
                          : activeSegment === "opportunity"
                            ? "Opportunities"
                            : activeSegment === "closed-lost"
                              ? "Closed / Lost"
        ...
        {/* --- DATAGRID --- */}
        <div
          id="contacts-data-grid"
          className="glass-panel border-slate-200 shadow-2xl overflow-hidden mb-10"
        >
            ...
            <table className="w-full table-fixed border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th
                    id="contacts-ai-rank-header"
                    onClick={() => handleSort("health_score")}
                    className="w-24 px-3 py-2 text-left text-[9px] font-black text-slate-500 uppercase tracking-widest cursor-pointer hover:text-slate-900 transition-colors group"
                  >
                    <div className="flex items-center gap-2">
                      A.I. RANK{" "}
                      <ChevronDown
                        size={14}
                        className={`transition-transform ${sortField === "health_score" ? "text-indigo-400" : "opacity-0"}`}
                      />
                    </div>
                  </th>
                  <th
                    onClick={() => handleSort("firstname")}
                    className="w-[36%] px-3 py-2 text-left text-[9px] font-black text-slate-500 uppercase tracking-widest cursor-pointer hover:text-slate-900 transition-colors"
                  >
                    PROFILE INFO
                  </th>
                  <th className="w-36 px-3 py-2 text-left text-[9px] font-black text-slate-500 uppercase tracking-widest">
                    STATUS TAG
                  </th>
                  <th className="w-40 px-3 py-2 text-left text-[9px] font-black text-slate-500 uppercase tracking-widest">
                    ORGANIZATION
                  </th>
                  <th
                    onClick={() => handleSort("last_modified")}
                    className="w-40 px-3 py-2 text-right text-[9px] font-black text-slate-500 uppercase tracking-widest cursor-pointer hover:text-slate-900 transition-colors"
                  >
                    LAST INTERACTION
                  </th>
                  <th className="w-48 px-3 py-2 text-center text-[9px] font-black text-indigo-400 uppercase tracking-widest">
                    QUICK COMMAND
                  </th>
                </tr>
              </thead>
                ...
                {loading ? (
                  Array.from({ length: 8 }).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td
                        colSpan={6}
                        className="px-3 py-4 border-b border-slate-200 bg-slate-50 opacity-70"
                      />
                    </tr>
                  ))
                ) : contacts.length === 0 ? (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-7 h-7 rounded-md border flex items-center justify-center text-[10px] font-black transition-all group-hover:rotate-6 ${
                              Number(contact.health_score ?? 0) >= 80
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5 min-w-0">
                              ...
                              <span className="text-[10px] text-slate-500 font-bold lowecase truncate">
                                {contact.email}
                              </span>
                            </div>
                          </div>
                      ...
                      <td className="px-3 py-2">
                        {contact.classification && (
                          <div className="flex flex-wrap items-center gap-2">
                            <div
                              className={`inline-flex items-center gap-2 px-2 py-0.5 rounded-md border text-[8px] font-black uppercase tracking-widest ${getStatusColor(contact.classification)}`}
                            >
                            ...
                            {(contact.classification === "Active Client" ||
                              contact.lifecyclestage === "customer") &&
                              /closed\s*lost/i.test(
                                contact.deal_stage || ""
                              ) && (
                            ...
                            {(isMeaningfulTag(contact.first_form) ||
                              isMeaningfulTag(contact.source)) && (
                      ...
                      <td className="px-3 py-2">
                        {contact.company ? (
                          <div className="flex items-center gap-2 group/company min-w-0">
                            <Building
                              size={16}
                              className="text-slate-600 group-hover/company:text-indigo-400 transition-colors"
                            />
                            <span className="text-slate-300 text-xs font-bold tracking-tight truncate">
                              {contact.company}
                            </span>
                          </div>
                        ) : (
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="text-xs font-black text-slate-900 tracking-tighter">
                          {formatDate(contact.last_modified)}
                        </div>
                        <div className="flex items-center justify-end gap-2 mt-1 flex-wrap">
                          {/* Data Quality Heuristics */}
                          {!contact.email && (
                            <span
                              className="text-[8px] font-black text-rose-400 uppercase tracking-widest bg-rose-500/10 px-2 py-0.5 rounded-md border border-rose-500/20"
                          ...
                          {!contact.hubspot_owner_id && (
                            <span
                              className="text-[8px] font-black text-amber-400 uppercase tracking-widest bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/20"
                          ...
                          {contact.last_modified &&
                            Date.now() -
                              new Date(contact.last_modified).getTime() >
                              90 * 24 * 60 * 60 * 1000 && (
                              <span
                                className="text-[8px] font-black text-slate-600 uppercase tracking-widest bg-slate-500/10 px-2 py-0.5 rounded-md border border-slate-500/20"
                                title="No activity in 90+ days"
                              >
                                Stale
                              </span>
                            )}
                          ...
                          {Number(contact.health_score ?? 0) >= 80 &&
                            contact.classification !== "Active Client" &&
                            contact.classification !== "Customer" &&
                            contact.classification !== "Employee" && (
                              <span className="text-[8px] font-black text-orange-400 uppercase tracking-widest bg-orange-500/10 px-2 py-0.5 rounded-md border border-orange-500/20">
                                Critical Lead 🔥
                              </span>
                            )}
                          {/* Ghost Opportunity */}
                          {contact.lifecyclestage === "opportunity" &&
                            ...
                            Date.now() -
                              ...
                              30 * 24 * 60 * 60 * 1000 && (
                              <span
                                className="text-[8px] font-black text-purple-400 uppercase tracking-widest bg-purple-500/10 px-2 py-0.5 rounded-md border border-purple-500/20"
                                title="Opportunity with no recent activity"
                              >
                                Ghost 👻
                              </span>
                            )}
                              ...
                              {[
                                {
                                  ...
                                  label: "LEAD",
                                  icon: <Users size={10} />,
                                  color: "hover:bg-slate-200 text-slate-600",
                                },
                                {
                                  ...
                                  color:
                                    "hover:bg-amber-100 text-amber-600 border-amber-200",
                                },
                                {
                                  ...
                                  color:
                                    "hover:bg-indigo-100 text-indigo-600 border-indigo-200",
                                },
                                {
                                  id: "opportunity",
                                  label: "OPP",
                                  icon: <Star size={10} />,
                                  color:
                                    "hover:bg-purple-100 text-purple-600 border-purple-200",
                                },
                                {
                                  id: "customer",
                                  label: "WON",
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleQuickStageChange(contact.id, s.id);
                                  }}
                                  className={`flex flex-col items-center justify-center w-8 h-8 rounded-lg border border-transparent transition-all ${s.color} active:scale-95`}
        ...
        <footer className="flex justify-between items-center py-6">
          <div className="flex items-center gap-4">
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
              Showing <span className="text-slate-900">{contacts.length}</span>{" "}
              / {pagination.total.toLocaleString()} Strategic Results
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="px-6 py-3 bg-indigo-500 text-slate-900 font-black rounded-2xl shadow-xl shadow-indigo-500/20 text-sm tracking-widest">
              {pagination.page}{" "}
              <span className="text-slate-900/50 mx-1">/</span>{" "}
          ...
          <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-3xl shadow-2xl p-8">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-2 text-3xl font-black text-slate-900">
                  {selectedContact.firstname || "Anonymous"}{" "}
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              {selectedContact.classification && (
                <span
                  className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-black uppercase tracking-widest ${getStatusColor(selectedContact.classification)}`}
                >
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm font-bold text-slate-900 mt-1">
                    Generate and push a note to HubSpot
                  </div>
                </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <button
                  ...
                  disabled={
                    noteStatus === "generating" || noteStatus === "saving"
                  }
            ...
            <div className="mt-6 rounded-2xl border border-slate-200 bg-indigo-50/30 p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    key={s.id}
                    onClick={() =>
                      handleQuickStageChange(selectedContact.id, s.id)
                    }
                    disabled={updatingStageId === selectedContact.id}
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all border ${
                      selectedContact.lifecyclestage === s.id
                        ? "bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-600/20"
                        : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"
                    } disabled:opacity-50`}
                  >
                    {updatingStageId === selectedContact.id &&
                    selectedContact.lifecyclestage === s.id ? (
      ...
      {showSaveModal && (
        <SaveSegmentModal
          ...
          queryConfig={{
            ...
            classification: ["hot", "nurture", "watch", "new"].includes(
              activeSegment
            )
              ...
              activeSegment === "mm"
                ...
                : activeSegment === "customer"
                  ...
                  : activeSegment === "opportunity"
                    ? "opportunity"
                    : undefined,
            dealType: searchTerm.startsWith("type:")
              ? searchTerm.replace("type:", "")
              : undefined,
            dealStage:
              activeSegment === "closed-lost" ? "closedlost" : undefined,
