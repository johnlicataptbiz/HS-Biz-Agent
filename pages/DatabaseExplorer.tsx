...
const DatabaseExplorer: React.FC = () => {
    ...
    const loadContacts = async () => {
        ...
        try {
            const resp = await fetch('/api/hubspot/crm/v3/objects/contacts?limit=50&properties=email,firstname,lastname,lifecyclestage,hubspot_owner_id');
            if (resp.ok) {
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="glass-card overflow-hidden border-white/5">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-white/5 bg-white/[0.02]">
                            <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest">Contact Identity</th>
                            <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest">Growth Phase</th>
                            <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest">Owner Mapping</th>
                            <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Operational Logic</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td colSpan={4} className="px-6 py-20 text-center">
                                    <div className="flex flex-col items-center gap-4">
                                        ...
                                        <p className="text-slate-500 font-black uppercase tracking-[0.3em] text-xs">Accessing Mirror Persistence Layer</p>
                                    </div>
                                </td>
                            </tr>
                                ...
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{contact.properties.email}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5">
                                ...
                                <td className="px-6 py-5">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-black text-slate-300 uppercase tracking-tighter italic">
                                            {contact.properties.hubspot_owner_id ? 'Assigned' : 'Unassigned'}
                                        </span>
                                    </div>
                                </td>
                                <td className="px-6 py-5 text-right">
                                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button title="Open in HubSpot" className="p-2 hover:bg-white/10 rounded-lg text-slate-500 hover:text-white transition-colors">
                                            <ExternalLink size={14} />
                                        </button>
                                        <button title="More Options" className="p-2 hover:bg-white/10 rounded-lg text-slate-500 hover:text-white transition-colors">
                ...
                <div className="p-6 border-t border-white/5 flex justify-between items-center bg-white/[0.01]">
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Displaying {filtered.length} of {contacts.length} Records</p>
                    <div className="flex gap-2">
                        <button className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-white transition-all">Previous</button>
                        <button className="px-4 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:bg-indigo-500/20 transition-all">Next Module</button>
                    </div>
                </div>
            </div>
        </div>
