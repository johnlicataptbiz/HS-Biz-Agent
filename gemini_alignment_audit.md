...
* **Update**: Build pipeline via `esbuild` established to bundle `@google/genai`.
...
* [x] Install `esbuild` and `@google/genai`.
* [x] Create `build.js` script to bundle functions.

### Step 2: `vibe-check.js` Upgrade

* [x] Implement `gemini-2.5-flash` model.
* [x] Paste `System Instructions`.
* [x] Paste `JSON Schema`.
* [x] Map `PRIVATE_APP_ACCESS_TOKEN` for CRM data fetching (Context) -> Pass to Gemini (Analysis).
...
* [ ] `hs secrets list` confirms keys are present (User Action Required).
* [x] Build process handles the bundling of the Google GenAI SDK.
* [ ] `vibe-check` card displays confident "AI" output.
