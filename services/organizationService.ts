...
export class OrganizationService {
    ...
    public async runStructuralScan(): Promise<{
        ...
        for (const obj of (schemas.results || [])) {
            ...
            const redundant = properties.filter(p => p.redundant).map(p => p.name);
    ...
    public async autoRepairAssociations(): Promise<{ fixed: number; failed: number }> {
        try {
            // 1. Fetch orphaned contacts (no primary company)
            const resp = await hubSpotService.request('/crm/v3/objects/contacts?limit=50&properties=email,website&associations=companies');
            if (!resp.ok) return { fixed: 0, failed: 0 };
            const data = await resp.json();
            
            const orphans = (data.results || []).filter((c: any) => !c.associations?.companies);
            let fixed = 0;
            let failed = 0;

            for (const contact of orphans) {
                const domain = contact.properties.email?.split('@')[1];
                if (!domain || ['gmail.com', 'outlook.com', 'yahoo.com'].includes(domain)) continue;

                // 2. Search for company by domain
                const searchResp = await hubSpotService.request('/crm/v3/objects/companies/search', {
                    method: 'POST',
                    body: JSON.stringify({
                ...
                if (matchedCompany) {
                    ...
                    if (aiData.matchFound && aiData.confidence > 0.8) {
                        ...
                        await hubSpotService.request(`/crm/v3/associations/contacts/companies/batch/create`, {
                            method: 'POST',
                            body: JSON.stringify({
                                inputs: [{
                                    from: { id: contact.id },
