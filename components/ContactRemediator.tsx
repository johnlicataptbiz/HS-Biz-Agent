...
const StrategicModelOptimizer: React.FC<ContactRemediatorProps> = ({ contactIds, contacts, onClose, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-4xl bg-white border border-slate-200 rounded-[2.5rem] shadow-[0_0_100px_rgba(99,102,241,0.25)] overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-8 border-b border-slate-200 flex justify-between items-center bg-gradient-to-r from-indigo-500/10 to-transparent">
           <div className="flex items-center gap-5">
              ...
              <div>
                <h2 className="text-2xl font-black tracking-tighter text-slate-900 uppercase italic leading-none">Strategic Model Optimizer</h2>
           ...
           <button onClick={onClose} className="p-3 hover:bg-slate-100 rounded-2xl transition-all text-slate-500 hover:text-slate-900 border border-transparent hover:border-slate-200">
 <span className="sr-only">Close</span>
          ...
          {analyzing ? (
            <div className="flex flex-col items-center justify-center py-24 gap-8">
               ...
               <div className="text-center">
                 <h3 className="text-2xl font-black text-slate-900 mb-3 uppercase tracking-tighter italic">Scanning Schema Integrity</h3>
            ...
            <div className="text-center py-24">
               ...
               <h3 className="text-2xl font-black text-slate-900 uppercase italic italic">Architectural Integrity: 100%</h3>
               <p className="text-slate-500 mt-2 font-medium">No strategic model debt detected in this cohort.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6">
              {tasks.map((task, idx) => (
                <div key={idx} className="group glass-panel p-8 border-slate-200 hover:border-indigo-500/40 transition-all duration-700 relative overflow-hidden bg-white shadow-xl">
                  ...
                  <div className="flex items-start gap-8 relative z-10">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-4">
                         <div>
                            <h4 className="font-black text-xl text-slate-900 uppercase tracking-tighter italic leading-none mb-1">{task.contactName}</h4>
                  ...
                  {task.status === 'completed' && (
                    <div className="absolute inset-0 bg-emerald-500/20 backdrop-blur-[2px] flex items-center justify-center animate-in zoom-in duration-300">
                       <div className="bg-emerald-500 text-slate-900 px-8 py-4 rounded-2xl flex items-center gap-4 font-black uppercase tracking-[0.2em] text-sm shadow-[0_20px_40px_rgba(16,185,129,0.3)] transform -rotate-2 border-2 border-white/30 scale-110">
        ...
        {/* Footer */}
        <div className="p-10 border-t border-slate-200 bg-slate-50 flex justify-between items-center">
           <div className="flex items-center gap-4">
              <div className="w-3 h-3 rounded-full bg-indigo-500 animate-pulse shadow-[0_0_10px_rgba(99,102,241,0.5)]" />
              <div>
                <p className="text-[11px] font-black text-slate-900 uppercase tracking-[0.1em]">Commercial Engine Synchronized</p>
 ...
 title="Abort Refinement"
                onClick={onClose}
                className="text-slate-500 hover:text-slate-900 font-black text-xs uppercase tracking-[0.2em] transition-all hover:tracking-[0.3em]"
                disabled={executing}
              >
                Abort Refinement
              </button>
 ...
 title="Align CRM Model"
                ...
                className="group relative px-10 py-4 bg-indigo-600 hover:bg-indigo-500 text-slate-900 rounded-3xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-indigo-600/30 transition-all hover:scale-105 active:scale-95 disabled:opacity-20 flex items-center gap-4 overflow-hidden"
              >
