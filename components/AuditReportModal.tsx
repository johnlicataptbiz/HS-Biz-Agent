...
const AuditReportModal: React.FC<AuditReportModalProps> = ({ isOpen, onClose, onRunAiRefinement }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
            <div 
                className="absolute inset-0 bg-white/80 backdrop-blur-md transition-opacity duration-500" 
                onClick={onClose}
                role="button"
            ...
            <div className="relative glass-card border-slate-200 w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-300 shadow-[0_0_100px_rgba(79,70,229,0.2)] max-h-[90vh] flex flex-col">
                ...
                <div className="p-8 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tighter uppercase italic">Comprehensive Direct Audit</h2>
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-slate-600">Deterministic Logic Scan</span>
                            </div>
                        </div>
                    </div>
                    <button 
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-10">
                    {loading ? (
                        <div className="h-64 flex flex-col items-center justify-center space-y-4">
                            ...
                            <p className="text-sm font-bold text-slate-600 uppercase tracking-widest animate-pulse">Scanning Portal Nodes...</p>
                        </div>
                    ) : report ? (
                        <>
                            {/* Score Overview */}
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                <div className="col-span-1 md:col-span-1 flex flex-col items-center justify-center p-8 bg-white/[0.03] border border-slate-200 rounded-[2.5rem] relative overflow-hidden group">
                                    ...
                                    <span className="text-5xl font-black text-slate-900 tracking-tighter mb-2">{report.overallScore}%</span>
                                ...
                                <div className="col-span-1 md:col-span-3 grid grid-cols-3 gap-4">
                                    {[
                                        ...
                                        <div key={i} className="p-6 bg-slate-50 border border-slate-200 rounded-3xl flex flex-col items-center justify-center gap-2">
                                            <s.icon size={20} className={s.color} />
                                            <span className="text-xl font-bold text-slate-900 tracking-tight">{s.score}%</span>
                            ...
                            <div className="glass-card border-indigo-500/20 bg-indigo-500/5 p-8 relative overflow-hidden group">
                                ...
                                <div className="space-y-4">
                                    ...
                                    {isSemanticLoading ? (
                                        <p className="text-sm text-slate-600 animate-pulse font-medium">Deconstructing portal architecture into semantic vectors...</p>
                                    ) : semanticReport ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                            <div className="space-y-3">
                                                 <p className="text-sm text-slate-900 font-bold italic leading-relaxed">"{semanticReport.analysis}"</p>
                                            ...
                                            <div className="space-y-3 border-l border-slate-200 pl-8">
                                                ...
                                                <button 
                                                     ...
                                                     className="text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:text-slate-900 transition-colors flex items-center gap-1"
                                                >
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    {report.issues.length > 0 ? report.issues.map((issue) => (
                                        <div key={issue.id} className="group p-6 bg-slate-50 border border-slate-200 hover:border-slate-200 rounded-[2rem] transition-all flex flex-col md:flex-row md:items-center justify-between gap-6">
                                            <div className="flex gap-5 max-w-xl">
                                                ...
                                                <div className="space-y-1">
                                                    <div className="flex items-center gap-3">
                                                        <h4 className="text-lg font-bold text-slate-900 group-hover:text-indigo-400 transition-colors uppercase tracking-tight italic">{issue.title}</h4>
                                            ...
                                            <button 
                                                ...
                                                className="px-6 py-4 glass-button border-indigo-500/20 text-indigo-300 hover:text-slate-900 hover:bg-indigo-500 text-xs font-black uppercase tracking-widest rounded-2xl flex items-center gap-2 group/btn"
                                            >
                                                {executing === issue.id ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center p-12 text-center space-y-4">
                                            ...
                                            <p className="text-xl font-bold text-slate-900 uppercase italic">Perfect Score Detected</p>
                ...
                <div className="p-8 bg-slate-50 border-t border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6 shrink-0">
                    <div className="flex items-center gap-4 text-left">
                        <div className="p-3 bg-indigo-500/10 rounded-xl">
                            <Sparkles size={18} className="text-indigo-400" />
                        </div>
                        <div>
                            <p className="text-xs font-extrabold text-slate-900 uppercase tracking-widest">Strategic Insight Upgrade</p>
                    ...
                    <div className="flex gap-4 w-full md:w-auto">
                        <button 
                            onClick={onClose}
                            className="flex-1 md:flex-none px-8 py-4 glass-button text-slate-600 font-black uppercase tracking-widest text-xs rounded-2xl"
                        >
                            Close Report
                        </button>
                        <button 
                            ...
                            className="flex-1 md:flex-none px-8 py-4 premium-gradient text-slate-900 font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl shadow-indigo-900/40 hover:scale-105 active:scale-95 transition-all"
                        >
                            Ask AI Co-Pilot
                        </button>
                    </div>
