...
const SCHEMA = {
  ...
  properties: {
    ...
    persona: {
      type: "STRING",
      description:
        "A catchy, strategic persona name (e.g. 'The Visionary Founder', 'The Tactical Gatekeeper')",
    },
    summary: {
      type: "STRING",
      description: "A concise 2-sentence executive summary of the lead's fit.",
    },
    strategicAdvice: {
      type: "STRING",
      description:
        "One specific, high-leverage closing tactic or engagement strategy.",
    },
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    ...
    return res
      .status(503)
      .json({ error: "Vibe AI Unavailable: Missing API Key" });
  }
  ...
  try {
    const model = genAI.getGenerativeModel({
      model: MODEL_NAME,
      systemInstruction:
        "You are Antigravity, the Strategic Architectural Lead. You provide ruthless, ROI-driven sales intelligence.",
      generationConfig: {
