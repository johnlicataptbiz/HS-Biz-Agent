export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === '/api/cleanup/ghost-workflows') {
        const wfResp = await fetch('https://api.hubapi.com/automation/v3/workflows', {
            headers: { 'Authorization': `Bearer ${hubspotToken}` }
        });
        const wfData = await wfResp.json();
        const ghosts = (wfData.workflows || []).filter((w) => w.enabled && (w.contactCounts?.enrolled || 0) === 0);
        
        const results = await Promise.all(ghosts.slice(0, 5).map(async (wf) => {
            const resp = await fetch(`https://api.hubapi.com/automation/v3/workflows/${wf.id}?action=pause`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${hubspotToken}` }
            });
            return resp.ok;
    ...
    if (action === '/api/cleanup/stalled-workflows') {
        ...
        const wfData = await wfResp.json();
        const stalled = (wfData.workflows || []).filter((w) => !w.enabled && (w.contactCounts?.active || 0) > 0);
    ...
    if (action === '/api/cleanup/properties') {
        const propResp = await fetch('https://api.hubapi.com/crm/v3/properties/contacts', {
            headers: { 'Authorization': `Bearer ${hubspotToken}` }
        });
