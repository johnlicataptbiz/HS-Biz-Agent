...
const hubspotTools = {
  list_newest_contacts: async (token) => {
    const resp = await fetch(
      "https://api.hubapi.com/crm/v3/objects/contacts?sort=-createdate&limit=5",
      {
  ...
  list_workflows: async (token) => {
    const resp = await fetch(
      "https://api.hubapi.com/automation/v3/workflows?properties=name,type,enabled",
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  get_contact: async (token, { id }) => {
    const resp = await fetch(
      `https://api.hubapi.com/crm/v3/objects/contacts/${id}?properties=email,firstname,lastname,jobtitle`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
...
const MCP_TOOLS_CONFIG = [
  {
    functionDeclarations: [
      ...
      },
      {
        name: "create_task",
        description:
          "Create a follow-up task in HubSpot, optionally associated with a contact.",
        parameters: {
          type: "OBJECT",
          properties: {
            id: {
              type: "STRING",
              description: "The contact ID to associate with (optional)",
            },
            subject: {
              type: "STRING",
            ...
            dueDate: {
              type: "STRING",
              description: "ISO 8601 timestamp for when the task is due",
            },
          },
...
const OPTIMIZE_SCHEMA = {
  ...
  properties: {
    specType: {
      type: "STRING",
      description:
        "One of: workflow_spec, sequence_spec, property_migration_spec, breeze_tool_spec.",
    },
    spec: {
      type: "OBJECT",
      properties: {
        ...
        apiCalls: {
          ...
          items: {
            ...
            properties: {
              ...
              body: {
                type: "STRING",
                description: "Stringified JSON object of the request body",
              },
...
const CLASSIFY_SCHEMA = {
  ...
  properties: {
    status: {
      type: "STRING",
      description:
        "One of the 9 Lead Statuses: New, Hot, Nurture, Watch, Unqualified, Past Client, Active Client, Rejected, Trash.",
    },
    tags: {
      type: "ARRAY",
      items: {
    ...
    inference: {
      type: "STRING",
      description:
        "Strategic inference based on notes and property data (max 2 sentences).",
    },
    strategicPriority: {
      type: "NUMBER",
      description: "Score from 0-100 indicating value to the business.",
    },
  },
...
const SENTIMENT_SCHEMA = {
  ...
  properties: {
    mood: {
      type: "STRING",
      description:
        "One word summary: e.g. Bullish, Reserved, Frustrated, Excited.",
    },
    score: {
      type: "NUMBER",
      description: "Market sentiment score 0-100 (100 = Extremely Positive).",
    },
    analysis: {
      type: "STRING",
      description: "Strategic summary of market sentiment (max 3 sentences).",
    },
    themes: {
      type: "ARRAY",
      items: { type: "STRING" },
      description: "Top 3 recurring themes detected in conversations.",
    },
  },
...
const OUTREACH_SCHEMA = {
  ...
  properties: {
    subject: {
      type: "STRING",
      description: "Catchy, personalized email subject line.",
    },
    body: {
      type: "STRING",
      description: "Professional, high-impact email body text.",
    },
    talkingPoints: {
      type: "ARRAY",
      items: { type: "STRING" },
      description: "Key themes used in the draft.",
    },
  },
...
const BRIEFING_SCHEMA = {
  ...
  properties: {
    ...
    slogan: {
      type: "STRING",
      description: "A high-impact, short operational slogan for the day.",
    },
  },
...
const FORECAST_SCHEMA = {
  ...
  properties: {
    projectedGrowth: {
      type: "NUMBER",
      description: "Percentage growth in deal volume next month.",
    },
    revenueVelocity: {
      type: "STRING",
      description:
        "Pace of deal closure: Accelerating, Stable, or Decelerating.",
    },
    topOpportunity: {
      type: "STRING",
      description: "The single biggest revenue opportunity detected.",
    },
    riskFactors: {
      type: "ARRAY",
      items: { type: "STRING" },
      description: "Top 2 risks to the forecast.",
    },
  },
  required: [
    "projectedGrowth",
      ...
      items: {
        ...
        properties: {
          ...
          issue: {
            type: "STRING",
            description:
              "Short description of the data quality issue detected.",
          },
          suggestedAction: {
            type: "STRING",
            enum: ["Update Status", "Archive", "Correct Data", "Re-assign"],
          },
          updates: {
            type: "OBJECT",
          ...
          reasoning: {
            type: "STRING",
            description:
              "Why this change is being recommended (max 1 sentence).",
          },
        },
        required: [
          "contactId",
    ...
    personas: {
      ...
      items: {
        ...
        properties: {
          ...
          targetListCriteria: {
            type: "STRING",
            description: "The HubSpot list filter criteria in plain English.",
          },
        },
        required: [
          "name",
          ...
          items: {
            ...
            properties: {
              ...
              operator: {
                type: "STRING",
                description:
                  "EQ, NEQ, LT, LTE, GT, GTE, CONTAINS_TOKEN, NOT_CONTAINS_TOKEN, IN, NOT_IN, HAS_PROPERTY, NOT_HAS_PROPERTY.",
              },
              value: { type: "STRING" },
            },
  ...
  try {
    ...
    if (typeof body === "string") {
      try {
        ...
        console.error("JSON Parse Error on body:", e);
      }
    }
    ...
    // 1. Validation Logic
    if (!prompt || !mode) {
      console.error("❌ AI Payload missing prompt or mode:", {
        mode,
        hasPrompt: !!prompt,
      });
      return res
        .status(400)
        .json({ error: "Missing prompt or mode in request body" });
    }
    ...
    if (!apiKey) {
      ...
      return res
        .status(503)
        .json({ error: "AI Service Unavailable: Missing API Key" });
    }
