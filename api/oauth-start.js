export default async function handler(req, res) {
  ...
  try { crypto.getRandomValues(bytes); } catch (e) { // Node polyfill
    for (let i = 0; i < bytes.length; i++) bytes[i] = Math.floor(Math.random() * 256);
  }
  const serverState = Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('');

  // Initialize global map if not present
  if (!global.__oauthStateMap) global.__oauthStateMap = {};
  global.__oauthStateMap[serverState] = {
    clientId: client_id || process.env.HUBSPOT_CLIENT_ID || process.env.VITE_HUBSPOT_CLIENT_ID || '7e3c1887-4c26-47a8-b750-9f215ed818f1',
    createdAt: Date.now(),
  ...
  const redirectUriFinal = redirect_uri || (req.headers.origin ? `${req.headers.origin}/` : '');
  const scopes = useMcp
