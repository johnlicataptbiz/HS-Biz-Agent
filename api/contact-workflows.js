...
export default async function handler(req, res) {
  ...
  try {
    ...
    const enrollResp = await hubspotFetch(`/automation/v2/workflows/enrollments/contacts/${contactId}`, hubspotToken);
    
    if (!enrollResp.ok) {
       // If 404, maybe no enrollments or just empty
       if (enrollResp.status === 404) return res.status(200).json({ workflows: [] });
       return res.status(enrollResp.status).json({ error: 'Failed to fetch enrollments' });
    }
